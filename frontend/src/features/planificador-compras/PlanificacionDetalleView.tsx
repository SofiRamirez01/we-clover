...
function formatearCantidad(cantidad: number, esKg: boolean): string {
  return `${cantidad.toLocaleString('es-AR', { maximumFractionDigits: 1 })} ${esKg ? 'g' : 'u.'}`;
}
            ...
            <>
              <div className="overflow-x-auto rounded-lg border border-wc-border bg-white">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {resumen.map((item) => (
                      <tr key={`${item.idTipoTela}::${item.idPaletaColor}`} className="border-b border-wc-border last:border-0">
                        ...
                        <td className="px-3 py-2 text-wc-text">{formatearCantidad(item.cantidad, item.unidadMedida === 'KG')}</td>
